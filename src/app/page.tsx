            ...
            <div className="relative md:absolute md:inset-0 flex items-center p-6 md:p-20 mt-[-60px] md:mt-0 z-10">
              <div className="glass bg-secondary/10 p-8 md:p-14 rounded-[24px] md:rounded-[32px] max-w-lg border-white/10 shadow-3xl">
                ...
                <p className="text-neutral-400 text-sm md:text-base leading-relaxed">
                  Nossa IA transforma fotos simples do seu celular em imagens que parecem ter saído de um estúdio profissional. Atraia mais clientes e aumente suas vendas no delivery hoje mesmo.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-6 md:px-8 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="space-y-8">
              <div className="hairline rounded-3xl overflow-hidden aspect-[3/4] bg-neutral-900 group">
                <img src={HERO_IMAGES[0]} className="w-full h-full object-cover transition-all duration-700" alt="Result" />
              </div>
              <div className="hairline rounded-3xl overflow-hidden aspect-square bg-neutral-900 group">
                <img src={HERO_IMAGES[1]} className="w-full h-full object-cover transition-all duration-700" alt="Result" />
              </div>
            </div>
            <div className="space-y-8 md:-translate-y-12">
              <div className="hairline rounded-3xl overflow-hidden aspect-[4/5] bg-neutral-900 group shadow-2xl shadow-primary/10">
                <img src={HERO_IMAGES[2]} className="w-full h-full object-cover transition-all duration-700" alt="Result" />
              </div>
              <div className="hairline rounded-3xl overflow-hidden aspect-[3/4] bg-neutral-900 group">
                <img src={HERO_IMAGES[3]} className="w-full h-full object-cover transition-all duration-700" alt="Result" />
              </div>
            </div>
            <div className="space-y-8">
              <div className="hairline rounded-3xl overflow-hidden aspect-square bg-neutral-900 group">
                <img src={HERO_IMAGES[4]} className="w-full h-full object-cover transition-all duration-700" alt="Result" />
              </div>
              <div className="hairline rounded-3xl overflow-hidden aspect-[3/4] bg-neutral-900 group">
                <img src={HERO_IMAGES[5]} className="w-full h-full object-cover transition-all duration-700" alt="Result" />
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto relative z-10">
            <div className="text-center mb-16 md:mb-32">
              ...
              >
                ...
                <span className="text-[10px] font-extrabold tracking-label text-primary uppercase">Investimento inteligente</span>
              </motion.div>
              ...
              <p className="text-neutral-500 text-lg md:text-xl font-medium">Pague apenas pelo que usar, sem taxas ocultas.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-stretch">
              {[
                { id: 'avulso', name: 'Single Shot', price: '19,90', credits: '10 créditos', desc: 'Para produções pontuais.' },
                { id: 'starter', name: 'The Studio', price: '49,90', credits: '40 créditos', desc: 'Pequenos cardápios.' },
                { id: 'pro', name: 'The Atelier', price: '99,90', credits: '120 créditos', desc: 'Fidelidade editorial completa.', popular: true },
                { id: 'scale', name: 'The Maison', price: '189,90', credits: '300 créditos', desc: 'Para impérios de delivery.' }
              ].map((plan) => (
                <div 
                  key={plan.id}
                  className={cn(
                  ...
                  {plan.popular && (
                    <>
                      ...
                      <motion.div
                        ...
                        style={{
                          background: "conic-gradient(from 0deg, transparent 0%, transparent 70%, var(--primary) 100%)",
                        }}
                      />
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    ...
                    <div className="mb-10 md:mb-12">
                      <div className="flex justify-between items-center mb-8 pb-2 border-b border-white/5">
                        ...
                        {plan.popular && (
                          <span className="bg-primary text-black text-[8px] font-black px-2 py-0.5 rounded-full tracking-wide">POPULAR</span>
                        )}
                      </div>
                      
                      <div className="flex flex-col gap-1">
                        <div className="flex items-baseline gap-1">
                          <span className="text-sm font-bold text-neutral-600 tracking-wide">R$</span>
                        ...
                        <div className={cn(
                          ...
                          plan.popular ? "bg-primary/10 text-primary border border-primary/20" : "bg-white/5 text-neutral-400"
                        )}>
                          {plan.credits}
                        </div>
                      </div>
                    ...
                    <ul className="space-y-5 mb-12 flex-grow">
                      {['Exportação em Alta Fidelidade', 'Resolução Retina (4K)', 'Isolamento de Objeto IA'].map((f) => (
                        <li key={f} className="flex items-center gap-4 text-[11px] text-neutral-400 font-bold tracking-wide uppercase">
                          <div className={cn(
                            "w-1.5 h-1.5 rounded-full shrink-0",
                            plan.popular ? "bg-primary shadow-[0_0_10px_rgba(var(--primary-rgb),0.8)]" : "bg-neutral-700"
                          )} /> {f}
                        </li>
                      ))}
                    </ul>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-8 grid grid-cols-2 md:grid-cols-4 gap-12 mb-20">
          ...
          {['Produto', 'Empresa', 'Legal', 'Social'].map((col) => (
            <div key={col} className="flex flex-col gap-4">
              <span className="text-[10px] font-extrabold text-white tracking-label mb-2 uppercase">{col}</span>
              <a href="#" className="text-[10px] text-neutral-500 hover:text-white transition-colors tracking-wide font-bold uppercase">Recursos</a>
              <a href="#" className="text-[10px] text-neutral-500 hover:text-white transition-colors tracking-wide font-bold uppercase">Sobre</a>
              <a href="#" className="text-[10px] text-neutral-500 hover:text-white transition-colors tracking-wide font-bold uppercase">Termos</a>
            </div>
          ))}
        </div>
        <div className="max-w-7xl mx-auto px-6 md:px-8 flex flex-col md:flex-row justify-between items-center gap-6 pt-12 border-t border-white/5 text-center md:text-left">
          ...
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/5">
              ...
              <span className="text-[9px] text-neutral-400 font-heading tracking-wide uppercase">Sistemas Operacionais</span>
            </div>
          </div>
        </div>
      </footer>
