...
export async function compressImage(imageFile: File): Promise<File> {
  ...
  try {
    console.info(
      `[Compression] Starting compression for: ${imageFile.name} (Original size: ${(imageFile.size / 1024 / 1024).toFixed(2)} MB)`
    );

    const compressedFile = await imageCompression(imageFile, options);

    console.info(
      `[Compression] Finished. New size: ${(compressedFile.size / 1024 / 1024).toFixed(2)} MB`
    );
